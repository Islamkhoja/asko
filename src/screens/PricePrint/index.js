...
import { ErrorModal, ConfirmModal, FilterModal, WarningModal, BusinessPartner, ImageModal, FilterModalReport, PrintModal } from '../../components/Modal';
...
const WarehouseBalanceReport = () => {
  ...
  const PrintModalRef = useCallback(ref => {
  ...
  const getItems = (pagination) => {
    ...
    axios
      .get(
        url + `/api/warehouse-without-movement?offset=${get(pagination, 'page', 1)}&status=false&U_branch=${get(pagination, 'whsCode', get(getMe, 'data.U_branch'))}&limit=${get(pagination, 'limit', limit)}&search=${get(pagination, 'value', '').toLowerCase()}&items=${actualData.map(item => `'${item.ItemCode}'`)}` + link,
        {
          headers: {
            'Authorization': `Bearer ${get(getMe, 'token')}`,
          }
          ...
          <div className='container'>
            <div className="order-head">
              <div className="order-main d-flex align justify">
                ...
                <button onClick={() => {
                  if (print.length) {
                    printRef.current?.open(print);
                  }
                }} style={{ background: '#E41B12' }} className={`btn-excel ${print.length ? '' : 'opacity-5'}`} >Печат</button>
                <button disabled={loading} className={`btn-excel ${loading && 'opacity-5'}`} onClick={() => exportTableToExcel({ mainData: print, warehouses, merchantList })}>Download as Excel</button>
              ...
              <div className='table-head'>
                <ul className='table-head-list d-flex align  justify'>
                  ...
                  <li className='table-head-item w-20'>
                    <input onClick={() => {
                      if (print.length == mainData.length) {
                        setPrint([])
                      }
                      else {
                        setPrint(mainData)
                      }
                    }} checked={mainData.length == print.length} type="checkbox" style={{ width: 17, height: 17 }} />
                  </li>
                </ul>
              </div>
                                ...
                                <div className='table-item-head d-flex align  justify'>
                                  ...
                                  <div className='w-20 p-16' >
                                    <button style={{ cursor: 'pointer' }}
                                      onClick={() => {
                                        if (!print.find(el => el.ItemCode == item.ItemCode)) {
                                          setPrint([...print, item])
                                        }
                                        else {
                                          setPrint(print.filter(el => el.ItemCode !== item.ItemCode))
                                        }
                                      }}
                                      className='btn-businesPartner'>
                                      <input checked={print.find(el => el.ItemCode == item.ItemCode)} type="checkbox" style={{ width: 17, height: 17 }} />
                                    </button>
                                  </div>
                                </div>
                              </li>
