...
const Order = () => {
  ...
  const getOrderByDocEntry = (doc, document) => {
    let link = `/api/getStockTransfers/${doc}/${document}`
    return axios
      .get(
        url + link,
        {
    ...
    axios
      ...
      .then(async ({ data }) => {
        if (get(docEntry, 'id') && !get(docEntry, 'status')) {
          getOrderByDocEntry(get(docEntry, 'id', 0), draft).then(async orderData => {
            ...
            setDate({
              ...
              ReqDate: moment(get(orderData, 'DocDueDate', '')).format("YYYY-MM-DD")
            })



            setSelectWh({
              WhsCode: get(orderData, 'ToWhsCode'),
              WhsName: get(orderData, 'ToWhsName'),
              ToWhsCode: get(orderData, 'Filler'),
              ToWhsName: get(orderData, 'WhsName')
            })
  ...
  const postOrder = async (status) => {
    ...
    if (!get(selectWh, 'WhsCode')) {
      warningNotify("Sklad tanlanmagan")
      return
    }
  ...
  const Orders = async () => {
    let link = '/api/stockTransferDrafts'
    setOrderLoading(true)
    let mapped = actualData
    let schema = {
      "DocDate": get(date, 'DocDate'),
      "DueDate": get(date, 'ReqDate'),
      "FromWarehouse": get(getMe, 'data.U_branch'),
      "ToWarehouse": get(selectWh, 'WhsCode'),
      "DocObjectCode": "67",
      "Comments": comment,
      U_branch: get(getMe, 'data.U_branch'),
      U_status: '1',
      "StockTransferLines": mapped.map(item => {
        let obj = {
          "ItemCode": get(item, 'ItemCode', ''),
          "Quantity": Number(get(item, 'value', 0)),
          "WarehouseCode": get(selectWh, 'WhsCode'),
          "FromWarehouseCode": get(getMe, 'data.U_branch')
        }
        return obj
      })
    }
            ...
            <div className="order-head">
              <div className="order-main d-flex align justify">
                <div className='d-flex align'>
                  <button onClick={() => navigate('/stock-transfers')} className='btn-back'>Назад</button>
                  <h3 className='title-menu'>Перемещение запасов</h3>
                </div>
                {!get(docEntry, 'id', 0) && <button onClick={() => postOrder(get(docEntry, 'id', 0) ? true : false)} className={`btn-head position-relative`}>
              ...
              <div className="order-head-data d-flex align justify">
                ...
                <div className='w-70'>
                  <input disabled={true} value={get(date, 'ReqDate', '')} onChange={(e) => setDate({ ...date, ReqDate: e.target.value })} type="date" className='order-inp' placeholder='Due Date' />
                </div>
              ...
              <div className="d-flex align justify">
                <div className='d-flex align'>
                  {/* ToWhsCode */}
                  {/* ToWhsName */}
                  <button style={{ marginRight: '20px', width: "200px" }} className='right-dropdown'>
                    <p className='right-limit-text'>{get(docEntry, 'id') ? get(selectWh, 'ToWhsCode') : get(getMe, 'data.U_branch')} - {warehouses.find(item => item.WhsCode == (get(docEntry, 'id') ? get(selectWh, 'ToWhsCode') : get(getMe, 'data.U_branch')))?.WhsName}</p>
                  </button>
                    ...
                    <button
                      onClick={() => {
                        let filterData = mainData.filter(el => {
                          return Number(el.value) > 0 && Number(get(el, 'OnHand.OnHand')) > 0
                        })
                            ...
                            <LazyLoad height={65} once>
                              <li key={i} className={`table-body-item`}>
                                <div className='table-item-head d-flex align  justify'>
                                  ...
                                  <div className='w-50 p-16' >
                                    <input
                                      disabled={Number(get(item, 'OnHand.OnHand')) == 0}
                                      ...
                                      type="text"
                                      className={`table-body-inp ${(Number(get(item, 'OnHand.OnHand', '')) <= 0) ? 'opacity-5' : ''
                                        }`}
                                      placeholder='-' />
                                  </div>
