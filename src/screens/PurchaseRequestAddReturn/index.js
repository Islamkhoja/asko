...
const Order = () => {
  ...
  const getCustomer = (customerDataObj) => {
    axios
      .get(
        url + `/api/vendors?search=${get(customerDataObj, 'customer', '').toLowerCase()}`,
        {
          headers: {
  ...
  const getOrderByDocEntry = (doc) => {
    let link = `/api/purchase-requests-return/${doc}`
    return axios
      .get(
        url + link,
        {
  ...
  const Update = () => {
    let link = `/api/PurchaseReturns/${get(docEntry, 'id', 0)}`
    let mapped = actualData
    let schema = {
      "CardCode": customerCode,
            ...
            <div className="order-head">
              <div className="order-main d-flex align justify">
                <div className='d-flex align'>
                  <button onClick={() => navigate('/purchase-return')} className='btn-back'>Назад</button>
                  <h3 className='title-menu'> Заявка на возврат </h3>
                </div>
