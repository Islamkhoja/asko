...
const PurchaseRequest = () => {
    ...
    const getOrders = (pagination) => {
        ...
        axios
            .get(
                url + `/api/purchase-request?offset=${get(pagination, 'page', 1)}&limit=${get(pagination, 'limit', limit)}&status=${false}&search=${get(pagination, 'value', '').toLowerCase()}` + link,
                {
                    headers: {
                        'Authorization': `Bearer ${get(getMe, 'token')}`,
                    }
                    ...
                    <div className='container'>
                        <div className='head'>
                            ...
                            <div className='right-head'>
                                ...
                                {<button onClick={() => navigate('/purchase-request-add')} className='btn-head'>
                                    Добавить
                                </button>}
                            </div>
                        </div>
                        <div className='table'>
                            <div className='table-head'>
                                <ul className='table-head-list d-flex align  justify'>
                                    ...
                                    <li className='table-head-item w-70'>Необходимый срок</li>
                                                    ...
                                                    return (
                                                        <li key={i} className={`table-body-item ${activeData === (i + 1) ? 'active-table' : ''}`}>
                                                            <div onClick={() => setActiveData(activeData === i + 1 ? 0 : (i + 1))} className='table-item-head d-flex align  justify'>
                                                                ...
                                                                <div className='w-70 p-16' >
                                                                    <p className='table-body-text w-70'>
                                                                        {moment(get(item, 'ReqDate', '')).format("DD.MM.YYYY")}
                                                                    </p>
                                                                </div>
                                                            ...
                                                            <div className='table-item-foot d-flex align'>
                                                                <button className='table-item-btn d-flex align'>
                                                                    {
                                                                        <Link className='table-item-text d-flex align' to={`/purchase-request-add/${get(item, 'DocEntry')}`}>
